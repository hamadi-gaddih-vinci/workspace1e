...
public class Temperatures {
    ...
    public int[] joursDeGel() {
        ...
        int [] tableDesJG = new int[nombreJoursDeGel()];
        int compteurJourGelee = 0;
        for (int i = 0; i < tableDesTemperatures.length; i++) {
            if (tableDesTemperatures[i] < 0) {
                tableDesJG[compteurJourGelee] = i++;
                compteurJourGelee++;
            }
        } return tableDesJG;
    }
    ...
    public boolean toutesPositives() {
        ...
        int i = 0;
        while (i < tableDesTemperatures.length && tableDesTemperatures[i] > 0){
            i++;
        }
        if (tableDesTemperatures.length == i)
            return true;
        return false;
    }
