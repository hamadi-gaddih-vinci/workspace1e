...
public class ABRDEntiers implements Iterable<Integer> {
	...
	private int nombrePositifsVR(NoeudEntier noeudEntier){
		...
		if (noeudEntier.entier < 0){
			return nombrePositifsVR(noeudEntier.droit);
		}
		if (noeudEntier.entier > 0)
			return 1 + nombrePositifsVR(noeudEntier.droit) + nombrePositifsVR(noeudEntier.gauche);
	...
	public boolean auMoins1PositifVR(NoeudEntier noeudEntier){
		if (noeudEntier == null)
			return false;
		if (noeudEntier.entier >= 0)
			return true;
