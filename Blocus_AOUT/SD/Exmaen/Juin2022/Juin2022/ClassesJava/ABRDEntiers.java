...
public class ABRDEntiers implements Iterable<Integer> {
	...
	private int nombreNegatifsVR(NoeudEntier noeudEntier){
		if (noeudEntier == null)
			return 0;
		if (noeudEntier.entier >= 0)
			return nombreNegatifsVR(noeudEntier.gauche);
		else {
			return 1 + nombreNegatifsVR(noeudEntier.gauche) + nombreNegatifsVR(noeudEntier.droit);
		}

		return nombreNegatifsVR(noeudEntier.gauche) + nombreNegatifsVR(noeudEntier.droit);
	}
