...
public class Equipe {
    ...
    public boolean insererApresChef(String nouvelEquipier){
        ...
        if (tete == queue){
            ...
            mapEquipierNoeud.put(nouvelEquipier, queue);
            return true;
        }
        Noeud noeudSuivant = tete.suivant;
        noeud.suivant = noeudSuivant;
        noeudSuivant.precedent = noeud;
    ...
    public boolean tronquerAPartir(String equipier){
        if(equipier==null||equipier.length()==0)
            throw new IllegalArgumentException();
        //TODO
        if (!mapEquipierNoeud.containsKey(equipier) || tete.equipier.equals(equipier))
            return false;
        queue = mapEquipierNoeud.get(equipier).precedent;
        Noeud baladeur = queue.suivant;
        while (baladeur != null){
