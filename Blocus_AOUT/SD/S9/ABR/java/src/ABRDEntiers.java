...
public class ABRDEntiers {
	...
	private boolean contient (NoeudEntier noeud, int entier){
		...
		else return contient(noeud.droit, entier);
	}
	...
	public void supprimeMin() throws ArbreVideException {
		...
		if (estVide())
			throw new ArbreVideException();
		supprimeMin(racine, liste);
	}
	private void supprimeMin(NoeudEntier noeud, ArrayList<Integer> liste){
		if (noeud.gauche == null) {
			liste.remove(noeud.entier);
			taille--;
		}
		else supprimeMin(noeud.gauche, liste);
