...
public class ExpressionArithmetique extends ArbreDeCaracteres {
	...
	public int nombreEntiersDifferents(){
		...
		nombreEntiersDifferents(racine, list);
		return list.size();
	}
	private void nombreEntiersDifferents(NoeudCaractere noeud, HashSet<Character> liste){
		if (noeud == null)
			return;
		if (noeud.caractere != '+' && noeud.caractere != '-' && noeud.caractere != '*' && noeud.caractere != '/')
			liste.add(noeud.caractere);
		nombreEntiersDifferents(noeud.gauche, liste);
		nombreEntiersDifferents(noeud.droit, liste);
	}
