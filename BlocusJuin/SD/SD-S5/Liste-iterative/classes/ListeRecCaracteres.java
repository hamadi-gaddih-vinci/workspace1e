...
public class ListeRecCaracteres {
    ...
    private boolean contient(NoeudCaractere noeudCaractere, char caractereRecherche) {
        ...
        if (noeudCaractere.caractere == caractereRecherche)
            return true;
        return contient(noeudCaractere.suivant, caractereRecherche);
    }
    ...
    private int nombreOccurrences(NoeudCaractere noeudCaractere, char caractereRecherche) {
        if (noeudCaractere == null)
            return 0;
        if (noeudCaractere.caractere == caractereRecherche)
            return 1 + nombreOccurrences(noeudCaractere.suivant, caractereRecherche);
    ...
    private void remplacer(NoeudCaractere noeudCaractere, char caractereARemplacer, char nouveauCaractere) {
        ...
        remplacer(noeudCaractere.suivant, caractereARemplacer, nouveauCaractere);
    }
    ...
    private void remplacerTout(NoeudCaractere noeudCaractere, char caractereARemplacer, char nouveauCaractere) {
        ...
        if (noeudCaractere.caractere == caractereARemplacer)
            noeudCaractere.caractere = nouveauCaractere;
        remplacerTout(noeudCaractere.suivant, caractereARemplacer, nouveauCaractere);
    }
    ...
    private char max(NoeudCaractere noeudCaractere) {
        if (noeudCaractere == null)
            return ' ';
        char m = max(noeudCaractere.suivant);
        if (noeudCaractere.caractere > m)
    ...
    private void enArrayList(NoeudCaractere noeudCaractere, ArrayList<Character> array) {
        if (noeudCaractere == null)
            return;
        array.add(noeudCaractere.caractere);
        enArrayList(noeudCaractere.suivant, array);
    }
    ...
    private boolean estEgalA(NoeudCaractere noeudCaractere, NoeudCaractere noeudCaractere2) {
        if (noeudCaractere == null && noeudCaractere2 == null)
            return true;
        if (noeudCaractere == null || noeudCaractere2 == null)
            return false;
        if (noeudCaractere.caractere != noeudCaractere2.caractere)
            return false;
        return estEgalA(noeudCaractere.suivant, noeudCaractere2.suivant);
    }
...
//	}
    ...
    public ListeRecCaracteres clone() {
        ...
        return clone(tete, listeRecCaracteres);
        // TODO
        // DEFI!
        // La methode recursive renvoie un noeud!
        // Lisez attentivement la version 1 de la solution de supprimerPremiereOccurrence
    }
	private ListeRecCaracteres clone(NoeudCaractere noeud, ListeRecCaracteres l){
        if (noeud == null)
            return null;
        l.tete = noeud;
        return clone(noeud.suivant, l);
	}
