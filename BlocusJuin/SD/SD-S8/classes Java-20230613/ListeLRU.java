...
public class ListeLRU<E> {
	...
	public boolean supprimer (E element) {
		...
		if (!contient(element))
			return false;
		Noeud noeudADelete = mapElementNoeud.get(element);
		Noeud avant = noeudADelete.precedent;
		Noeud apres = noeudADelete.suivant;

		avant.suivant = apres;
		apres.precedent = avant;
		mapElementNoeud.remove(element, noeudADelete);
		return true;
	}
	...
	public E supprimerDernier() {
		...
		if (taille() == 0)
			return null;
		Noeud supp = queue.precedent;
		Noeud av = supp.precedent;
		av.suivant = queue;
		queue.precedent = av;
