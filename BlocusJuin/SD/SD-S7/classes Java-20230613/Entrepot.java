...
public class Entrepot {
	...
	private HashMap<Integer, Societe> mapEnt;
	...
	public Entrepot(int nombreHangars) {
		...
		// TODO
		mapEnt = new HashMap<Integer, Societe>();
	...
	public Societe getSociete(int numeroSociete){
		//TODO
		return mapEnt.get(numeroSociete);
	}
	...
	public int attribuerHangar(int numeroSociete, String nomSociete) {
		...
		if (nombreHangarsLibres == 0)
			return -1;
		Societe societe = mapEnt.get(numeroSociete);
		if (societe == null){
			societe = new Societe(numeroSociete, nomSociete);
			mapEnt.put(numeroSociete, societe);
		}
		int numAttributionHangar = numeroSociete%nombreHangars;

		if (nombreHangarsLibres > 0){
			for (int i = numAttributionHangar; i < tableHangars.length; i++) {
			...
			for (int i = 0; i < numAttributionHangar; i++) {
				if (tableHangars[i].getSociete() == null){
					societe.ajouterHangar(i);
					tableHangars[i].setSociete(societe);
					nombreHangarsLibres--;
